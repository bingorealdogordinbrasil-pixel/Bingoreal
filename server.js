...
let jogo = { 
    bolas: [], 
    fase: "acumulando", 
    premioAcumulado: 0.00, // COMEÇA EM ZERO
    tempoSegundos: 300, 
    ganhador: null, 
    valorGanho: 0,
    totalVendasRodada: 0 
...
// ... (Loop do jogo e Sorteio mantidos iguais) ...
setInterval(async () => {
...
async function reiniciarGlobal() {
    try {
        ...
        jogo = { bolas: [], fase: "acumulando", premioAcumulado: 0.00, tempoSegundos: 300, ganhador: null, valorGanho: 0, totalVendasRodada: 0 };
    }
}
...
app.post('/admin/dashboard', async (req, res) => {
    ...
    try {
        ...
        res.json({ jogadores, saques, lucroRodada: (jogo.totalVendasRodada - jogo.premioAcumulado), premioAcumulado: jogo.premioAcumulado, vendasRodada: jogo.totalVendasRodada });
...
// --- ROTA DE SAQUE (SEM FICAR NEGATIVO) ---
app.post('/solicitar-saque', async (req, res) => {
    const { userId, valor, chavePix } = req.body;
    const v = parseFloat(valor);
    try {
        const user = await User.findById(userId);
        if (v > user.valorLiberadoSaque || v > user.saldo) return res.status(400).json({ error: "Saldo insuficiente!" });
...
// ... (Restante das rotas: login, registro, pix e bonus permanecem iguais)
